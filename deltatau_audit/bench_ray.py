...
def run_remote_audit(
    adapter_factory: Callable[[], Any], env_factory: Callable[[], Any], n_episodes: int = 10, seed: int = 42
):
    ...
    result = run_full_audit(adapter, env_factory, n_episodes=n_episodes, verbose=False, seed=seed)
    return result
...
class RayClusterAuditor:
    ...
    def mass_audit(self, jobs: List[Dict[str, Any]], episodes_per_job: int = 50) -> List[Dict[str, Any]]:
        """
        ...
        for i, job in enumerate(jobs):
            futures.append(
                run_remote_audit.remote(
                    job["adapter_factory"], job["env_factory"], n_episodes=episodes_per_job, seed=42 + i
                )
            )
