...
class RecursiveSelfArchitectureExperiment:
    ...
    def run(self, out_dir: Path) -> Dict[str, float]:
        ...
        summary = {
            **return_stats,
            "computation_efficiency": float(return_stats["mean_return"] / (np.mean(avg_steps) + 1e-8)) if avg_steps else 0.0,
            "complexity_adaptivity": float(stress_adaptation),
            "composite_score": float(normalised * stress_adaptation),
        }
